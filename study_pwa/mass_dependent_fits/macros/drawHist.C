const TString mainDir = "/d/grid17/septian/EtaPi0Analysis/study_pwa/mass_dependent_fits/";
const TString dirRootFlatTree = "/d/grid17/septian/EtaPi0Analysis/study_pwa/mass_dependent_fits/rootFiles/";
// TString rootHistFitResult = "/d/grid17/septian/EtaPi0Analysis/study_pwa/mass_dependent_fits/etapi_plot_D2++_pD2++.root";
// TString dirRootFlatTree = "/d/grid17/septian/EtaPi0Analysis/study_pwa/mass_dependent_fits/rootFiles/t010020_m104172_test/";
const Int_t NPol = 4;
...
void drawHist(){
    ...
    if (isOpenFlatTrees){
        ...
        for (Int_t iTBin=0;iTBin<NTBin;iTBin++){
            ...
            for (Int_t iPol=0;iPol<NPol;iPol++){
                ...
                Float_t Mpi0eta,Mpi0p;
                Float_t vanHove_omega, vanHove_x, vanHove_y, pVH;
                Float_t weight;

                TTree *tFlatTreeSignal = (TTree*)fFlatTreeSignal[iPol][iTBin]->Get("kin");

                tFlatTreeSignal->SetBranchStatus("*",0);
                tFlatTreeSignal->SetBranchStatus("Mpi0eta",1);
                tFlatTreeSignal->SetBranchStatus("Mpi0p",1);
                tFlatTreeSignal->SetBranchStatus("vanHove_omega",1);
                tFlatTreeSignal->SetBranchStatus("vanHove_x",1);
                tFlatTreeSignal->SetBranchStatus("vanHove_y",1);
                tFlatTreeSignal->SetBranchStatus("pVH",1);
                tFlatTreeSignal->SetBranchStatus("weightASBS",1);

                tFlatTreeSignal->SetBranchAddress("Mpi0eta",&Mpi0eta);
                tFlatTreeSignal->SetBranchAddress("Mpi0p",&Mpi0p);
                tFlatTreeSignal->SetBranchAddress("vanHove_omega",&vanHove_omega);
                tFlatTreeSignal->SetBranchAddress("vanHove_x",&vanHove_x);
                tFlatTreeSignal->SetBranchAddress("vanHove_y",&vanHove_y);
                tFlatTreeSignal->SetBranchAddress("pVH",&pVH);
                tFlatTreeSignal->SetBranchAddress("weightASBS",&weight);
                ...
                for (Int_t iEvent=0;iEvent<tFlatTreeSignal->GetEntries();iEvent++){
                    ...
                    if (isPlotVanHopeAngle) {
                        h2_Mpi0Eta_VanHove_sig[iPol][iTBin]->Fill(Mpi0eta,vanHove_omega,weight);
                        h2_VanHove_XY_sig[iPol][iTBin]->Fill(vanHove_x,vanHove_y,weight);
                        h1_Mpi0p_sig[iPol][iTBin]->Fill(Mpi0p,weight);
                        if (pVH) h1_Mpi0p_pVH_sig[iPol][iTBin]->Fill(Mpi0p,weight);
                    }
                    if (isPlotMPi0Eta) h1_Mpi0eta_sig[iPol][iTBin]->Fill(Mpi0eta,weight);
                }

                TTree *tFlatTreeBkgnd = (TTree*)fFlatTreeBkgnd[iPol][iTBin]->Get("kin");
                tFlatTreeBkgnd->SetBranchStatus("*",0);
                tFlatTreeBkgnd->SetBranchStatus("Mpi0eta",1);
                tFlatTreeBkgnd->SetBranchStatus("Mpi0p",1);
                tFlatTreeBkgnd->SetBranchStatus("vanHove_omega",1);
                tFlatTreeBkgnd->SetBranchStatus("vanHove_x",1);
                tFlatTreeBkgnd->SetBranchStatus("vanHove_y",1);
                tFlatTreeBkgnd->SetBranchStatus("pVH",1);
                tFlatTreeBkgnd->SetBranchStatus("weightASBS",1);

                tFlatTreeBkgnd->SetBranchAddress("Mpi0eta",&Mpi0eta);
                tFlatTreeBkgnd->SetBranchAddress("Mpi0p",&Mpi0p);
                tFlatTreeBkgnd->SetBranchAddress("vanHove_omega",&vanHove_omega);
                tFlatTreeBkgnd->SetBranchAddress("vanHove_x",&vanHove_x);
                tFlatTreeBkgnd->SetBranchAddress("vanHove_y",&vanHove_y);
                tFlatTreeBkgnd->SetBranchAddress("pVH",&pVH);
                tFlatTreeBkgnd->SetBranchAddress("weightASBS",&weight);

                for (Int_t iEvent=0;iEvent<tFlatTreeBkgnd->GetEntries();iEvent++){
                    tFlatTreeBkgnd->GetEntry(iEvent);
                    if (isPlotVanHopeAngle) {
                        h2_Mpi0Eta_VanHove_bkg[iPol][iTBin]->Fill(Mpi0eta,vanHove_omega,weight);
                        h2_VanHove_XY_bkg[iPol][iTBin]->Fill(vanHove_x,vanHove_y,weight);
                        h1_Mpi0p_bkg[iPol][iTBin]->Fill(Mpi0p,weight);
                        if (pVH) h1_Mpi0p_pVH_bkg[iPol][iTBin]->Fill(Mpi0p,weight);
                    }
                    if (isPlotMPi0Eta) h1_Mpi0eta_bkg[iPol][iTBin]->Fill(Mpi0eta,weight);
                }
    ...
    if (isPlotA2CS) {
        for (const auto dir: fitResultDir) {
            TString fName = dir + etaPiPlotterOutLogName;
            auto AccCorrYields = GetAccCorrYield(fName);
        }
    }
}
...
Double_t GetAccCorrYield(TString fileName){
    ...
    const TString strTotalEvent = "EVENTS";
    const TString strD = "D";

    // search if there is "TOTAL EVENTS" in the file
    TString line;
    // TString lastLine;

    Bool_t readFlag = kFALSE;
    list<TString> lines;
    ...
    for (auto pline = lines.cbegin(); pline != lines.cend(); ++pline) {
        if (pline->EqualTo(strFitFracWave)) {
            // cout << "Found " << strD << " in the file" << endl;
            TString tempStr = *(++(++pline));
